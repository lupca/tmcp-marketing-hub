...
export default function ProductsServicesPage() {
    ...
    const filtered = useMemo(() => {
        ...
        return items.filter(i =>
            ...
            i.usp?.toLowerCase().includes(query)
        );
    }, [items, search]);
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center p-12 text-center text-gray-500">
                        ...
                        <p>Add your first product or service to get started.</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full">
                            <thead className="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">USP</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {filtered.map(item => (
                                    <tr key={item.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-500 max-w-xs truncate" title={item.usp || ''}>
                    ...
                    <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" value={form.brand_id} onChange={e => setForm({ ...form, brand_id: e.target.value })}>
                        ...
                        <div>
                            ...
                            <textarea className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 h-24" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Unique Selling Proposition (USP)</label>
                            <textarea className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 h-20" value={form.usp} onChange={e => setForm({ ...form, usp: e.target.value })} />
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex gap-2 mb-2">
                                ...
                                />
                                <button className="px-3 py-2 bg-gray-100 border border-gray-300 rounded-md hover:bg-gray-200 text-sm" onClick={addFeature}>Add</button>
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {form.key_features.map((f, i) => (
                                    <span key={i} className="inline-flex items-center gap-1 px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-sm">
                                        {f}
                                        <button onClick={() => removeFeature(i)} className="hover:text-blue-900"><X size={14} /></button>
                                    </span>
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex gap-2 mb-2">
                                ...
                                />
                                <button className="px-3 py-2 bg-gray-100 border border-gray-300 rounded-md hover:bg-gray-200 text-sm" onClick={addBenefit}>Add</button>
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {form.key_benefits.map((b, i) => (
                                    <span key={i} className="inline-flex items-center gap-1 px-3 py-1 bg-green-50 text-green-700 rounded-full text-sm">
                                        {b}
                                        <button onClick={() => removeBenefit(i)} className="hover:text-green-900"><X size={14} /></button>
                                    </span>
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Default Offer</label>
                            <input className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" value={form.default_offer} onChange={e => setForm({ ...form, default_offer: e.target.value })} placeholder="e.g., Free trial for 30 days" />
                        </div>
                    </div>
                </Modal>
            )}
